...
#         merged_df2 = pd.DataFrame(data = anno_filt2 , columns = df.columns)
#         merged_df2.to_csv(output_folder + "/" + "cheby1/" + csv_file , index = False)
#         merged_df3 = pd.DataFrame(data = anno_filt3 , columns = df.columns)
#         merged_df3.to_csv(output_folder + "/cheby2/" + csv_file , index = False)
#         merged_df4 = pd.DataFrame(data = anno_filt4 , columns = df.columns)
...
print("clean_signal.shape", clean_signal.shape, "noise_data.shape", noise_data.shape)
...
def add_noise(clean_signal, noise_data):
    ...
    for i in range(num_segments):
        noisy_signal[:, i] = clean_signal_array[:, i] + 0.0002 * noise_data_array[:, i]
    return noisy_signal
...
save_to_csv(clean_plus_noise, noisy_signal)
