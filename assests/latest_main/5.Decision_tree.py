...
def dt_model_function( local_features_file , description : str = ""):
    ...
    dt_model = Ml_helper('dt', random_state = rand_state , criterion = split_criteria)
    dt_model.classifier.fit(x_train , y_train)
    dt_model.k_fold_strat_crossval(x_train , y_train , k , rand_state)
    y_pred = dt_model.test_n_results(x_test , y_test , description)
    ...
    if 'PatientID' in features_df.columns:
        ...
        dataset[[x[0] for x in classification_rep if x[2] == 0] ].to_csv(path_or_buf = 'Stage_2/dt/clean.csv',index = False)
        dataset[[x[0] for x in classification_rep if x[2] == 1] ].to_csv(path_or_buf = 'Stage_2/dt/partly_corrupted.csv',index = False)
        dataset[[x[0] for x in classification_rep if x[2] == 2] ].to_csv(path_or_buf = 'Stage_2/dt/corrupted.csv',index = False)
        ...
        for i in range(len(class_list)):
            ...
            for j in range(len(class_list)):
                ...
                if len(temp) > 0:
                    ...
                    dataset[temp].to_csv(path_or_buf = f'4.missclassifications/dt/{description}_{i}_to_{j}.csv',index = False)
...
print("\n~~~~~ dt:: W/O AE FEATURES ~~~~~")
list_stats , stat_miss_class = dt_model_function(stats_features, description = "Statistical features")

print("\n~~~~~ dt:: WITH ALL FEATURES ~~~~~")
list_all , all_miss_class = dt_model_function(feature_merged, description = "All features")
