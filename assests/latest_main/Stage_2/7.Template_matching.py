...
ppg_df = pd.read_csv('clean.csv',skiprows=1)
...
for _,col in ppg_df.items():
    for i in range(num_windows):
        ...
        correlation_values = correlate(window.reshape(-1,1), template_data.reshape(-1,1), mode='full')/(np.std(template_data) * np.std( window))
        lags = correlation_lags(len(window), len(template_data), mode='full')
        ...
        print("Pearson's correlation coefficient: ",Pearson_correl(window,template_data))

        # Visualize the correlation result
        if Pearson_correl(window,template_data) > 0.4:
